            ...
            <div className="mt-2 space-y-2">
              <div className="flex flex-wrap gap-2">
                <Button
                  variant="outline"
                  size="sm"
                  onClick={loadV57MeterTestSample}
                ...
                <Button
                  variant="outline"
                  size="sm"
                  onClick={loadV57MillimeterTestSample}
                  ...
                  <div className="mt-2 space-y-2">
                    {parsedCargo.dimensions.map((dim, index) => (
                      <div key={`${dim.length}-${dim.width}-${dim.height}-${index}`} className="flex items-center justify-between p-2 bg-gray-50 rounded text-sm">
                        <span>{dim.length} × {dim.width} × {dim.height} cm</span>
                        <span className="text-muted-foreground">
                          {dim.quantity} {parsedCargo.packageType === 'pallets' ? '托盘' :
                                          parsedCargo.packageType === 'boxes' ? '箱' : '件'}
                        </span>
                        <span className="font-medium">
                          {((dim.length * dim.width * dim.height * dim.quantity) / 1000000).toFixed(3)} cbm
                        </span>
                      </div>
                    ))}
                  </div>
